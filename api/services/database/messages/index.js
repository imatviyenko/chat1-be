...
async function getMessages(chatGuid, userEmail, sequenceNumberAfter, sequenceNumberBefore) {
    ...
    // in this case to improve user expirience we fetch top latest config.returnUnfilteredMessagesLimit messages without filters
    return Message.find({}).lean().sort({ $natural: -1 }).limit(config.returnUnfilteredMessagesLimit).exec();
}
